...
import AuthPage      from "./api/pages/AuthPage"
import RecruiterView from "./api/pages/RecruiterView"
...
const T = { display:"'Inter', sans-serif", mono:"'JetBrains Mono', monospace" }

function ScoreRing({ score, size=80 }) {
  const radius=( size-10)/2, circum=2*Math.PI*radius
  ...
  useEffect(()=>{
    ...
    const animate=ts=>{
      ...
      setDrawn((1-Math.pow(1-prog,3))*score)
  ...
  return(
    <svg width={size} height={size} style={{transform:"rotate(-90deg)"}}>
      <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={C.border} strokeWidth={6}/>
      <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={color} strokeWidth={6}
              strokeDasharray={circum} strokeDashoffset={offset} strokeLinecap="round"
              style={{filter:`drop-shadow(0 0 6px ${color})`}}/>
      <text x={size/2} y={size/2} textAnchor="middle" dominantBaseline="central"
            fill={color} fontSize={size*0.22} fontWeight={600} fontFamily={T.mono}
...
function UploadZone({ label, accept, onChange, uploaded }) {
  ...
  return(
    <div onClick={()=>inputRef.current.click()}
         ...
         style={{border:`1.5px dashed ${drag?C.accent:uploaded?C.greenBord:C.border}`,
                 borderRadius:"10px",padding:"20px",textAlign:"center",cursor:"pointer",
                 background:drag?C.accentLo:uploaded?C.greenBg:C.surfaceHi,transition:"all 0.2s"}}>
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:"4px",alignItems:"center"}}>
        ...
        <span style={{fontSize:"11px",color,fontFamily:T.mono,fontWeight:600}}>{score}%</span>
      </div>
...
function ResultCard({ result, rank, onViewDetail, isSelected }) {
  const scoreColor=result.final_score>=70?C.green:result.final_score>=50?C.yellow:C.red
  return(
    <div style={{border:`1px solid ${isSelected?C.accent:C.border}`,borderRadius:"12px",
                 padding:"20px",background:isSelected?"#0f1729":C.surface,transition:"all 0.2s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"16px"}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"4px"}}>
            <span style={{fontSize:"10px",fontFamily:T.mono,color:C.textMut,
                          border:`1px solid ${C.border}`,borderRadius:"4px",padding:"1px 6px"}}>
              #{rank}
            </span>
      ...
      <button onClick={()=>onViewDetail(result.jd_id)}
              style={{background:"none",border:`1px solid ${C.borderHi}`,borderRadius:"6px",
                      color:C.accentText,fontSize:"12px",fontFamily:T.mono,
        ...
        {isSelected?"close breakdown ×":"full breakdown →"}
      </button>
    </div>
  )
}
...
function DetailPanel({ result, onClose }) {
  ...
  const Section=({title,children})=>(
    <div style={{marginBottom:"16px"}}>
      <div style={{fontSize:"10px",fontFamily:T.mono,color:C.textMut,letterSpacing:"0.1em",
                   marginBottom:"8px",borderBottom:`1px solid ${C.border}`,paddingBottom:"6px"}}>
        {title}
      </div>
      {children}
    </div>
  ...
  return(
    <div style={{border:`1px solid ${C.accent}`,borderRadius:"12px",padding:"20px",
                 background:"#0c1220",boxShadow:`0 0 30px ${C.accent}22`}}>
...
function StudentView({ authUser, onLogout }) {
  ...
  const handleResumeUpload=async()=>{
    ...
    setError("");setUploading(u=>({...u,resume:true}))
    try{
  ...
  const handleJDUpload=async()=>{
    ...
    setError("");setUploading(u=>({...u,jd:true}))
    try{
    ...
    }catch{setError("JD upload failed.")}
    finally{setUploading(u=>({...u,jd:false}))}
  }

  const handleMatch=async()=>{
    if(!resumeId||jdList.length===0){setError("Upload a resume and at least one JD first");return}
    setError("");setMatching(true);setMatchResults(null);setSelectedResult(null)
    try{
      const res=await authFetch("http://127.0.0.1:8000/match/batch",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({resume_id:resumeId,jd_ids:jdList.map(j=>j.id)})
      })
      setMatchResults(await res.json())
    }catch{setError("Matching failed. Check the backend.")}
    finally{setMatching(false)}
  }

  const handleViewDetail=async(jd_id)=>{
    if(selectedJdId===jd_id){setSelectedResult(null);setSelectedJdId(null);return}
    try{
      const res=await authFetch(`http://127.0.0.1:8000/match/${resumeId}/${jd_id}`)
      setSelectedResult(await res.json());setSelectedJdId(jd_id)
  ...
  const inputStyle={width:"100%",padding:"9px 12px",boxSizing:"border-box",
    ...
    color:C.textPri,fontSize:"13px",fontFamily:T.display,outline:"none"}

  const btnStyle=primary=>({padding:"10px 18px",borderRadius:"8px",
    border:primary?"none":`1px solid ${C.border}`,
    background:primary?C.accent:C.surfaceHi,
    color:primary?"white":C.textSec,
    fontSize:"13px",fontWeight:500,fontFamily:T.display,cursor:"pointer",transition:"all 0.15s"})

  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:T.display,
      ...
      <div style={{borderBottom:`1px solid ${C.border}`,padding:"0 24px",
        ...
        <span style={{fontFamily:T.mono,fontWeight:600,fontSize:"15px",letterSpacing:"-0.02em"}}>
          dev<span style={{color:C.accent}}>lens</span>
        </span>
        {resumeId&&(
          <span style={{marginLeft:"12px",fontSize:"11px",fontFamily:T.mono,color:C.textMut,
        ...
        <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:"10px"}}>
          ...
          <button onClick={onLogout} style={{background:"none",border:`1px solid ${C.border}`,
                                             ...
                                             fontFamily:T.mono,padding:"4px 10px",cursor:"pointer"}}>
            sign out
          </button>
        </div>
      </div>
      ...
      <div style={{maxWidth:"660px",margin:"0 auto",padding:"32px 24px"}}>
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,
          ...
          <div style={{fontSize:"10px",fontFamily:T.mono,color:C.textMut,
                       letterSpacing:"0.1em",marginBottom:"14px"}}>01 — RESUME</div>
          <UploadZone label="Drop your resume here or click to browse" accept="application/pdf"
                      onChange={e=>setResumeFile(e.target.files[0])} uploaded={resumeName}/>
          {!resumeId&&(
        ...
        <div style={{background:C.surface,border:`1px solid ${C.border}`,
                     borderRadius:"12px",padding:"20px",marginBottom:"12px"}}>
          <div style={{fontSize:"10px",fontFamily:T.mono,color:C.textMut,
                       letterSpacing:"0.1em",marginBottom:"14px"}}>02 — JOB DESCRIPTIONS</div>
          ...
          <UploadZone label="Drop JD PDF here or click to browse" accept="application/pdf"
                      onChange={e=>setJdFile(e.target.files[0])} uploaded={jdFile?.name}/>
          ...
          {jdList.length>0&&(
            <div style={{marginTop:"14px",display:"flex",flexDirection:"column",gap:"6px"}}>
              {jdList.map(jd=>(
                <div key={jd.id} style={{display:"flex",justifyContent:"space-between",
                                          alignItems:"center",background:C.greenBg,
                                          border:`1px solid ${C.greenBord}`,
                                          borderRadius:"8px",padding:"8px 12px"}}>
                  <div>
                    <span style={{fontSize:"13px",color:C.textPri,fontWeight:500}}>{jd.title}</span>
        ...
        <button onClick={handleMatch}
                ...
                style={{width:"100%",padding:"14px",
                        background:(!resumeId||jdList.length===0||matching)?C.surfaceHi:C.accent,
                        border:"none",borderRadius:"10px",
                        color:(!resumeId||jdList.length===0||matching)?C.textMut:"white",
                        fontSize:"15px",fontWeight:600,fontFamily:T.display,
                        cursor:(!resumeId||jdList.length===0||matching)?"not-allowed":"pointer",
                        transition:"all 0.2s",marginBottom:"24px",
                        boxShadow:(!resumeId||jdList.length===0||matching)
                          ?"none":`0 0 20px ${C.accent}44`}}>
          {matching?"Analysing..."
            :!resumeId?"Upload a resume to continue"
            :jdList.length===0?"Add at least one JD"
        ...
        {matchResults&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",
              ...
              <span style={{fontSize:"16px",fontWeight:600,color:C.textPri}}>Results</span>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:"12px"}}>
              {matchResults.results?.map((r,i)=>(
                <div key={r.jd_id}>
                  <ResultCard result={r} rank={i+1} onViewDetail={handleViewDetail}
...
export default function App() {
  const [authUser,setAuthUser]=useState(()=>{
    ...
    return{token,user_id:localStorage.getItem("user_id"),
           email:localStorage.getItem("email"),role:localStorage.getItem("role")||"student"}
  })
  ...
  const handleAuth=data=>{
    ...
    setAuthUser({token:data.access_token,user_id:data.user_id,email:data.email,role:data.role})
  }

  const handleLogout=()=>{
    ["token","user_id","email","role"].forEach(k=>localStorage.removeItem(k))
    setAuthUser(null)
  }

  if(!authUser)return<AuthPage onAuth={handleAuth}/>
  if(authUser.role==="recruiter")return<RecruiterView authUser={authUser} onLogout={handleLogout}/>
  return<StudentView authUser={authUser} onLogout={handleLogout}/>
}